...
{                                                   // no guarantee all given pages are mapped
  ...
  while (fpit != NULL && pgit < pgnum) {
    ...
    /* Move to the next frame and page */
    fpit = fpit->fp_next;
    pgit++;
  }
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
{
  int pgit;
  int fpn;
  struct framephy_struct *newfp_str, *fp_head = NULL;
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++)
  {
        ...
        __swap_out_page(caller, &fpn) == 0)
    {
      ...
      newfp_str->fpn = fpn;
      newfp_str->fp_next = fp_head;
      fp_head = newfp_str;
    }
    else
    { 
      /* Not enough frames, return error code */
      /* Free any allocated frames before returning */
      while (fp_head != NULL) {
        struct framephy_struct *tmp = fp_head;
        MEMPHY_put_freefp(caller->mram, fp_head->fpn);
        fp_head = fp_head->fp_next;
        free(tmp);
      }
  ...
  /* Return list of allocated frames */
  *frm_lst = fp_head;
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  /* Set VMA0 next to NULL as it's the only VMA initially */
  vma0->vm_next = NULL;
  ...
  /* Initialize the FIFO page list for replacement */
  mm->fifo_pgn = NULL;
...
int print_pgtbl(struct pcb_t *caller, uint32_t start, uint32_t end)
{
  ...
  /* Print human-readable mapping information */
  for (pgit = pgn_start; pgit < pgn_end; pgit++) {
